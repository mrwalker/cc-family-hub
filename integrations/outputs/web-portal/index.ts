...
export default class WebPortalOutput extends BaseIntegration implements OutputIntegration {
  ...
  async publishPlan(plan: WeeklyPlan): Promise<void> {
    mkdirSync(PORTAL_DATA_DIR, { recursive: true });
    // Write as a JS file so the portal works without a server (no CORS on file://)
    const js =
      `// Auto-generated by Family Hub planner — do not edit manually\n` +
      `window.PLAN_DATA = ${JSON.stringify(plan, null, 2)};\n`;
    writeFileSync(join(PORTAL_DATA_DIR, "plan-data.js"), js, "utf8");
    console.log(`  Web portal updated: outputs/web-portal/index.html`);
  }
